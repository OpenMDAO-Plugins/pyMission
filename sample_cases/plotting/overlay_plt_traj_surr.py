...
lengths = [100, 200, 300, 400, 500, 750, 1000, 1500,
           2000, 2500, 3000, 3500, 4000, 4500, 5000,
           5500, 6000, 6500, 7000, 7500, 8000, 8500,
           9000]
NCURVES = 23
num_lines = range(NCURVES)
folder_path = 'range_sweep_data'
dists = []
alts = []
machs = []
speeds = []
...
for index in xrange(num_plts):
    flight_length = lengths[index]
    print "reading data for %i case" % flight_length
    # name = 'mission_history_range_%i.bson' % flight_length
    ...
    # file_name = os.path.join(folder_path, 'range_%04i' % flight_length)
    ...
    file_name = os.path.join(folder_path, 'range_%04i.npy' % flight_length)
    ...
    #  SFC] = np.loadtxt(folder_path+name)
    dist = dist/(1e3 * 1.852)
...
for index in xrange(num_plts):
    ...
    for i, a in enumerate(subax):
        # print i, index, lengths[index], dists[index].shape, values[i].shape
...
subfig.savefig('./plots/SciTech_CarpetRange.pdf')
